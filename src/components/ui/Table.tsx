  ...
  return (
    <div id={id} className="w-full overflow-x-auto border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xs">
      <table className="w-full text-xs text-slate-800 dark:text-slate-200 border-collapse min-w-[650px]">
        <thead className="bg-slate-100/95 dark:bg-slate-800/95 text-slate-700 dark:text-slate-200 uppercase font-extrabold text-xs tracking-wider border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10 backdrop-blur-xs">
          <tr>
            {columns.map((col) => (
              <th
                key={col.key}

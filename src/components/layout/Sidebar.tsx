...
export const EmployeeSidebar: React.FC<{ unreadNotificationCount?: number }> = ({ unreadNotificationCount = 2 }) => {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-300 border-r border-slate-200 dark:border-slate-800/80 flex flex-col shrink-0 min-h-[calc(100vh-4rem)] transition-colors duration-200">
      <div className="px-4 py-3.5 uppercase text-xs font-extrabold tracking-widest text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800/60 flex items-center justify-between">
        ...
        {employeeNavItems.map((item) => (
          ...
          >
            ...
            {item.badge !== undefined && (
              <span className={`text-xs px-2 py-0.5 rounded-full shadow-xs ${item.badgeColor}`}>
                {item.badge}
              </span>
            )}
          </NavLink>
  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-300 border-r border-slate-200 dark:border-slate-800/80 flex flex-col shrink-0 min-h-[calc(100vh-4rem)] transition-colors duration-200">
      ...
      <nav className="flex-1 px-3 py-3 space-y-1 overflow-y-auto">
        ...
        <div className="pt-5 pb-2 px-3.5 uppercase text-xs font-extrabold tracking-widest text-slate-500 dark:text-slate-400 flex items-center justify-between">

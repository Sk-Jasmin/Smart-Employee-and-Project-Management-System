...
class AuthService {
  async login(credentials: LoginCredentials): Promise<{ user: User; token: string }> {
    try {
      ...
      if (error.response?.status === 401 || error.response?.data?.message?.toLowerCase().includes('bad credentials')) {
        throw new Error('The details are invalid. Please try again.');
      }
      console.warn('Backend API connection failed or returned error, executing fallback auth handler...', error.message);

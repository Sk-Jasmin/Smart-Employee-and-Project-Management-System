          ...
          <div className="bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white p-3.5 flex items-center justify-between text-xs shadow-inner">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-pink-100">{todayBirthday.department} • Send your wishes!</span>
              </div>
            </div>
            {!wishedIds.includes(todayBirthday.id) ? (
              ...
              <span className="text-[10px] bg-white/30 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
          ...
          {UPCOMING_BIRTHDAYS.map((person) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="font-bold text-xs text-slate-900 dark:text-slate-100 leading-tight flex items-center gap-1.5">
                      {person.name}
                      {person.isToday && (
                        <span className="bg-pink-600 text-white text-[9px] font-extrabold uppercase px-1.5 py-0.2 rounded-full">
                          Today!
                        </span>
                      )}
                    </h5>
                ...
                <div>
                  {isCelebrating ? (
                    <span className="text-[11px] font-bold text-pink-600 dark:text-pink-400 flex items-center gap-1 animate-pulse">
      ...
      {selectedPerson && (
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-slate-500 font-bold">Quick Greetings:</span>
              <button
                onClick={() => setWishMessage("Happy Birthday! Wishing you a fantastic year filled with success and happiness! 🎉🎂")}
                className="text-[10px] bg-slate-100 dark:bg-slate-800 hover:bg-pink-100 dark:hover:bg-pink-950 px-2 py-1 rounded text-slate-700 dark:text-slate-300 cursor-pointer"
              >
                Classic 🎂
              </button>
              <button
